...
import { getAnnouncement, saveAnnouncement, deleteAnnouncement } from '@/services/api';
        ...
        {/* Announcements Tab */}
          {activeTab === 'Announcements' && (
            <div className="dashboard-content">
              <div className="dashboard-section-title">📣 Site Announcement</div>
              <p style={{color:'var(--text-muted)', fontFamily:'var(--font-mono)', fontSize:'0.82rem', marginBottom:'24px'}}>
                This message pops up in the bottom-right corner for every visitor the first time they open the site in a session. Use it for maintenance notices, new novel announcements, or anything you want readers to see.
              </p>

              {annMsg && (
                <div className={`upload-success${annMsg.startsWith('Error') ? ' upload-error' : ''}`} style={{marginBottom:'16px'}}>
                  {annMsg}
                </div>
              )}

              <form onSubmit={handleSaveAnn} className="upload-form">
                ...
                <div className="form-group">
                  <label style={{display:'flex', alignItems:'center', gap:'10px', cursor:'pointer', userSelect:'none'}}>
                    ...
                    <span style={{fontWeight:600}}>Show announcement to visitors</span>
                  </label>
                  <div style={{fontSize:'0.78rem', color:'var(--text-muted)', marginTop:'4px', marginLeft:'26px'}}>
                    Uncheck to hide without deleting the message.
                  </div>
                </div>
                ...
                <div className="form-group">
                  ...
                  <div style={{display:'flex', gap:'8px', flexWrap:'wrap'}}>
                    {[
                      {v:'info',         label:'ℹ️ Info',         color:'#3b82f6'},
                      {v:'warning',      label:'⚠️ Warning',       color:'#f59e0b'},
                      {v:'maintenance',  label:'🔧 Maintenance',   color:'#8b5cf6'},
                      {v:'success',      label:'✅ Good News',      color:'#22c55e'},
                      {v:'announcement', label:'📣 Announcement',  color:'#ff6b2b'},
                    ].map(t => (
                      <button key={t.v} type="button"
                        onClick={() => setAnn(a => ({...a, type: t.v}))}
                        style={{
                          padding:'7px 14px', borderRadius:'8px', cursor:'pointer',
                          fontFamily:'var(--font-mono)', fontSize:'0.78rem', fontWeight:600,
                          border: ann.type === t.v ? `2px solid ${t.color}` : '1px solid var(--border)',
                          background: ann.type === t.v ? `${t.color}22` : 'var(--bg-dark)',
                          color: ann.type === t.v ? t.color : 'var(--text-muted)',
                ...
                <div className="form-group">
                  <label>Message *</label>
                  <textarea required rows={4} value={ann.message}
                    onChange={e => setAnn(a => ({...a, message: e.target.value}))}
                    placeholder="e.g. The server will be in maintenance for 2 hours starting 10 PM UTC. Thank you for your patience!"
                    style={{resize:'vertical'}} />
                  <div style={{fontSize:'0.75rem', color:'var(--text-muted)', marginTop:'4px'}}>
                    {ann.message.length} characters
                  </div>
                </div>
                ...
                <div className="form-group">
                  ...
                  <input type="url" value={ann.link}
                    onChange={e => setAnn(a => ({...a, link: e.target.value}))}
                    placeholder="https://discord.gg/... (shows a 'Learn more' button in the banner)" />
                </div>
                ...
                {ann.message && (
                  <div className="form-group">
                    ...
                    <div style={{
                      ...
                      border:`1px solid ${{info:'#3b82f6',warning:'#f59e0b',maintenance:'#8b5cf6',success:'#22c55e',announcement:'#ff6b2b'}[ann.type]||'#3b82f6'}`,
                      borderLeft:`4px solid ${{info:'#3b82f6',warning:'#f59e0b',maintenance:'#8b5cf6',success:'#22c55e',announcement:'#ff6b2b'}[ann.type]||'#3b82f6'}`,
                      background:'var(--bg-dark)',
                    }}>
                      <span style={{fontSize:'1.1rem'}}>{{info:'ℹ️',warning:'⚠️',maintenance:'🔧',success:'✅',announcement:'📣'}[ann.type]||'ℹ️'}</span>
                      <div style={{flex:1}}>
                        <p style={{margin:0, fontSize:'0.84rem', fontFamily:'var(--font-mono)', color:'var(--text-primary)'}}>{ann.message}</p>
                        {ann.link && <a href={ann.link} style={{fontSize:'0.76rem', color:'var(--accent-orange)'}}>Learn more →</a>}
                      </div>
                      <div style={{width:'14px', height:'14px', background:'var(--border)', borderRadius:'2px'}}/>
                    </div>
                  </div>
                )}
                ...
                <div className="form-actions">
                  <button type="submit" className="btn-primary" disabled={annLoading}>
                    {annLoading ? 'Saving…' : '💾 Save & Publish'}
                  </button>
                  <button type="button" className="btn-secondary" onClick={handleDeleteAnn} disabled={annLoading}
...
const TABS = ['Overview', 'My Novels', 'Upload Novel', 'Analytics', 'Announcements'];
...
export default function DashboardPage() {
  ...
  // ── Announcement state ───────────────────────────────────────────────────
  const [ann,        setAnn]        = useState({ message:'', type:'info', link:'', active:false });
  const [annLoading, setAnnLoading] = useState(false);
  const [annMsg,     setAnnMsg]     = useState('');
  const [annLoaded,  setAnnLoaded]  = useState(false);

  useEffect(() => {
    if (activeTab === 'Announcements' && !annLoaded) {
      getAnnouncement()
        .then(d => { if (d) setAnn({ message: d.message||'', type: d.type||'info', link: d.link||'', active: !!d.active }); setAnnLoaded(true); })
  ...
  async function handleSaveAnn(e) {
    ...
    try {
      ...
      setAnnMsg('Announcement saved and live!');
  ...
  async function handleDeleteAnn() {
    ...
    try {
      ...
      setAnn({ message:'', type:'info', link:'', active:false });
      setAnnMsg('Announcement cleared.');
    } catch (err) { setAnnMsg('Error: ' + err.message); }
    setAnnLoading(false);
    setTimeout(() => setAnnMsg(''), 3000);
  }
