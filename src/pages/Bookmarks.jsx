...
export function toggleBookmark(novel) {
  ...
  const idx  = list.findIndex(b => b.novelId === novel._id);
  if (idx !== -1) {
    list.splice(idx, 1);
  } else {
    list.unshift({
...
export default function Bookmarks() {
  ...
  return (
    <>
      <SEO title="My Bookmarks — idenwebstudio" description="Your saved novels" />
        ...
        <div className="container">
          ...
          {bookmarks.length === 0 ? (
            ...
            <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--text-muted)' }}>
              ...
              <Link to="/browse" className="btn-primary" style={{ display: 'inline-flex' }}>Browse Novels</Link>
            </div>
          ) : (
            <>
              {/* Controls */}
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '20px' }}>
                ...
                <div style={{ position: 'relative', flex: '1', minWidth: '180px' }}>
                  ...
                  />
                  {search && (
                    <button onClick={() => setSearch('')} style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '1.1rem', lineHeight: 1 }}>×</button>
                  )}
                </div>
                {/* Sort */}
                <select
                      ...
                      <div style={{ flex: 1, padding: '12px 14px', minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '10px' }}>
                          <Link
                            ...
                            style={{ fontSize: '0.72rem', padding: '5px 12px', display: 'inline-flex', alignItems: 'center', gap: '4px' }}
                          >
                            <svg width="11" height="11" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                            Read
                          </Link>
                          <button
                            onClick={() => remove(b.novelId)}
