    ...
    <div className="browse-page">
      ...
      <div className="container">
        ...
        <div className="browse-bar">
          <form className="browse-search-wrap" onSubmit={handleSearch}>
          ...
          <button className={`browse-genre-toggle ${genre?'has-filter':''}`} onClick={() => setShowGenres(s => !s)}>
            ...
            {genre || 'Genre'}
            {genre && (
              <span className="browse-clear-x" onClick={e => { e.stopPropagation(); setGenre(''); setPage(1); }}>✕</span>
            )}
          </button>
        </div>
        ...
        {showGenres && (
          <div className="browse-genre-row">
            ...
            {GENRES.map(g => (
              <button key={g} className={`genre-chip ${genre===g?'active':''}`}
                onClick={() => { setGenre(g===genre?'':g); setPage(1); setShowGenres(false); }}>{g}</button>
            ))}
          </div>
        )}
